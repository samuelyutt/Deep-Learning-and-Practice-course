...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		// TODO
		// Return the index of weight table
		size_t idx = 0;
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				// Estimate value of move state = r + V(s')
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		// TODO
		float next_move_value = 0.0;
		path.pop_back();
		while (path.size()) {
			state& move = path.back();
			// td_error = r + V(s'next) - V(s')
			float td_error = move.reward() + next_move_value - move.value();
			next_move_value = move.reward() + update(move.after_state(), alpha * td_error);
			path.pop_back();
		}
	}
...
int main(int argc, const char* argv[]) {
	...
	tdl.load("../weights/04051337/weights.bin");
